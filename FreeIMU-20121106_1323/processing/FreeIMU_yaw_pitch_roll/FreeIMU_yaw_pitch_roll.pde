...
void setup() 
{
  ...
  myPort = new Serial(this, "/dev/ttyUSB0",38400);  
...
void serialEvent(Serial p) {
  if(p.available() >= 18) {
    ...
    if (inputString != null && inputString.length() > 0) {
      //from arduIMU: !!!VER:1.9,RLL:-0.11,PCH:-0.04,YAW:-0.01,IMUH:253,MGX:98,MGY:-233,MGZ:-235,MGH:67,TOW:21522100***
...
void getMagHeading(float[] q, float[] ypr){
  float Head_X;
  float Head_Y;
  float cos_roll;
  float sin_roll;
  ...
  Head_X = q[4]*cos_pitch+q[5]*sin_roll*sin_pitch+q[6]*cos_roll*sin_pitch;
