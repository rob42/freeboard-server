 ...
 void SCP1000_StartAcquisition()
{
  ...
  Wire.write((uint8_t)SNS_ADDR_POPERATION);  // Start acquisition
  Wire.write((uint8_t)SCP_MODE); 
...
{
  Wire.beginTransmission((uint8_t)PRESSURE_ADDR);
  Wire.write((uint8_t)SNS_ADDR_POPERATION);  
  Wire.write(0x00);   // Stop acquisition
  Wire.endTransmission();
}
