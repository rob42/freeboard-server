	...
	public void doAfterCompose(Window comp) throws Exception {
		...
		if(logger.isDebugEnabled())logger.debug("Init..");
			if(Util.getConfig(null).containsKey(Constants.DOUBLE_LOG_X)){
				...
				if(logger.isDebugEnabled())logger.debug("  doubleLog location set to "+doubleLog.getLeft()+", "+doubleLog.getTop());
			}else{
				doubleLog.setPosition("left,center");
				if(logger.isDebugEnabled())logger.debug("  doubleLog location set to "+doubleLog.getPosition());
			}
