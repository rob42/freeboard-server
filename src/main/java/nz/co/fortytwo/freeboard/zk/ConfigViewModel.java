...
public class ConfigViewModel extends SelectorComposer<Window> {
	...
	public void cfgSaveClick(MouseEvent event) {
		...
		try {
			...
			if (isValidDepthUnit(temp)) {
				config.setProperty(Constants.DEPTH_UNIT,
						(String) (cfgDepthUnit.getSelectedItem().getValue()));
				Util.saveConfig();
				// notify others
				...
				Messagebox
						.show("Depth unit must be 'feet' 'Fathoms' or 'meters'");
			}
	...
	private boolean isValidBaudRate(String value) {
		...
		if (rate == 4800 || rate == 9600 || rate == 19200 || rate == 38400
				|| rate == 57600)
			return true;
		return false;
	}
	...
	private void setConfigDefaults() {
		try {
			...
			cfgDepthOffset.setValue(config
					.getProperty(Constants.DEPTH_ZERO_OFFSET));
			portsToScan.setValue(config.getProperty(Constants.SERIAL_PORTS));
			cfgWindOffset.setValue(config
					.getProperty(Constants.WIND_ZERO_OFFSET));
