...
L.EditToolbar.Edit = L.Handler.extend({
	...
	save: function () {
		...
		this._featureGroup.eachLayer(function (layer) {
			var oldLatLng = this._uneditedLayerProps[L.Util.stamp(layer)].latlng;
			if(layer.getLatLng().lat!=oldLatLng.lat
					|| layer.getLatLng().lng!=oldLatLng.lng){
				editedMarkers.push([layer.getLatLng().lat,layer.getLatLng().lng,oldLatLng.lat,oldLatLng.lng]);
			}
		}, this);
		
		this._map.fire('draw:edited', { layers: editedMarkers });
	},
